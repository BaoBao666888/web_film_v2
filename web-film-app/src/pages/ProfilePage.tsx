...
export function ProfilePage() {
  ...
  const [themePreference, setThemePreference] = useState<ThemePreference>(
    (user?.theme_preference as ThemePreference) || getStoredThemePreference()
  );
  ...
  const handleThemeSelect = async (option: ThemePreference) => {
    if (!authUser) return;
    ...
    try {
      ...
      setThemeStatus({
        type: "error",
        message:
          err instanceof Error ? err.message : "Không thể lưu giao diện.",
      });
    } finally {
  ...
  return (
    <div className="space-y-10">
      <PageHeader
        ...
        description="Thông tin cá nhân và danh sách phim yêu thích của bạn."
        actions={
        ...
        <div className="space-y-5 rounded-3xl border border-white/10 bg-white/5 p-6 shadow-xl shadow-black/25">
          ...
          <div className="rounded-2xl border border-white/10 bg-dark/60 p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-sm font-semibold text-white">Giao diện</p>
                <p className="text-xs text-slate-400">
                  Chọn giao diện ưa thích, hệ thống sẽ lưu ngay.
                </p>
              </div>
              {themeSaving && (
